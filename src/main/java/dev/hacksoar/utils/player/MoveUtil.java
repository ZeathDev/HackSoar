...
public class MoveUtil {
    ...
    public boolean isMoving() {
        return HackSoar.mc.thePlayer.moveForward != 0 || HackSoar.mc.thePlayer.moveStrafing != 0;
    }
    ...
    public boolean enoughMovementForSprinting() {
        return Math.abs(HackSoar.mc.thePlayer.moveForward) >= 0.8F || Math.abs(HackSoar.mc.thePlayer.moveStrafing) >= 0.8F;
    }
    ...
    public boolean canSprint(final boolean legit) {
        return (legit ? HackSoar.mc.thePlayer.moveForward >= 0.8F
                && !HackSoar.mc.thePlayer.isCollidedHorizontally
                && (HackSoar.mc.thePlayer.getFoodStats().getFoodLevel() > 6 || HackSoar.mc.thePlayer.capabilities.allowFlying)
                && !HackSoar.mc.thePlayer.isPotionActive(Potion.blindness)
                && !HackSoar.mc.thePlayer.isUsingItem()
                && !HackSoar.mc.thePlayer.isSneaking()
    ...
    public double movementDelta() {
        return Math.hypot(HackSoar.mc.thePlayer.posX - HackSoar.mc.thePlayer.prevPosX, HackSoar.mc.thePlayer.posZ - HackSoar.mc.thePlayer.prevPosZ);
    }

    public double speedPotionAmp(final double amp) {
        return HackSoar.mc.thePlayer.isPotionActive(Potion.moveSpeed) ? ((HackSoar.mc.thePlayer.getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1) * amp) : 0;
    }
    ...
    public double jumpBoostMotion(final double motionY) {
        if (HackSoar.mc.thePlayer.isPotionActive(Potion.jump)) {
            return motionY + (HackSoar.mc.thePlayer.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1F;
        }
    ...
    public static javax.vecmath.Vector2d getMotion(double moveSpeed) {
        MovementInput movementInput = HackSoar.mc.thePlayer.movementInput;

        double moveForward = movementInput.moveForward;
        double moveStrafe = movementInput.moveStrafe;
        double rotationYaw = HackSoar.mc.thePlayer.movementYaw;
    ...
    public int depthStriderLevel() {
        return EnchantmentHelper.getDepthStriderModifier(HackSoar.mc.thePlayer);
    }
    ...
    public static float fallDistanceForDamage() {
        ...
        if (HackSoar.mc.thePlayer.isPotionActive(Potion.jump)) {
            int amplifier = HackSoar.mc.thePlayer.getActivePotionEffect(Potion.jump).getAmplifier();
    ...
    public void forward(final double speed) {
        ...
        HackSoar.mc.thePlayer.motionX = -Math.sin(yaw) * speed;
        HackSoar.mc.thePlayer.motionZ = Math.cos(yaw) * speed;
    }
    ...
    public double getAllowedHorizontalDistance() {
        ...
        if (HackSoar.mc.thePlayer.isInWeb) {
        ...
        } else if (HackSoar.mc.thePlayer.isSneaking()) {
    ...
    public void jumpRandom(final double motion) {
        HackSoar.mc.thePlayer.motionY = motion + (Math.random() / 500);
    }
    ...
    public void strafe(final double speed) {
        ...
        final double yaw = direction();
        HackSoar.mc.thePlayer.motionX = -MathHelper.sin((float) yaw) * speed;
        HackSoar.mc.thePlayer.motionZ = MathHelper.cos((float) yaw) * speed;
    }
    ...
    public void strafe(final double speed, float yaw) {
        ...
        HackSoar.mc.thePlayer.motionX = -MathHelper.sin(yaw) * speed;
        HackSoar.mc.thePlayer.motionZ = MathHelper.cos(yaw) * speed;
    }
     ...
     */
    public void stop() {
        HackSoar.mc.thePlayer.motionX = 0;
        HackSoar.mc.thePlayer.motionZ = 0;
    }
    ...
    public double direction() {
        float rotationYaw = HackSoar.mc.thePlayer.movementYaw;

        if (HackSoar.mc.thePlayer.moveForward < 0) {
            rotationYaw += 180;
        }
        ...
        if (HackSoar.mc.thePlayer.moveForward < 0) {
            forward = -0.5F;
        } else if (HackSoar.mc.thePlayer.moveForward > 0) {
            forward = 0.5F;
        }

        if (HackSoar.mc.thePlayer.moveStrafing > 0) {
    ...
    public double wrappedDirection() {
        ...
        if (HackSoar.mc.thePlayer.moveForward < 0 && HackSoar.mc.thePlayer.moveStrafing == 0) {
            rotationYaw += 180;
        }
     ...
     */
    public double speed() {
        return Math.hypot(HackSoar.mc.thePlayer.motionX, HackSoar.mc.thePlayer.motionZ);
    }

    public void setMoveEvent(final EventMove moveEvent, final double moveSpeed) {
        setMoveEvent(moveEvent, moveSpeed, HackSoar.mc.thePlayer.movementYaw, HackSoar.mc.thePlayer.movementInput.moveStrafe, HackSoar.mc.thePlayer.movementInput.moveForward);
    }
    ...
    public void fixMovement(final EventMoveInput event, final float yaw) {
        ...
        final double angle = MathHelper.wrapAngleTo180_double(Math.toDegrees(MoveUtil.direction(HackSoar.mc.thePlayer.rotationYaw, forward, strafe)));
    ...
    public double getMCFriction() {
        ...
        if (HackSoar.mc.thePlayer.onGround) {
            f = HackSoar.mc.theWorld.getBlockState(new BlockPos(MathHelper.floor_double(HackSoar.mc.thePlayer.posX), MathHelper.floor_double(HackSoar.mc.thePlayer.getEntityBoundingBox().minY) - 1, MathHelper.floor_double(HackSoar.mc.thePlayer.posZ))).getBlock().slipperiness * 0.91F;
        }
    ...
    public double[] moveFlying(float strafe, float forward, final boolean onGround, final float yaw, final boolean sprinting) {
        ...
        final float playerWalkSpeed = HackSoar.mc.thePlayer.getAIMoveSpeed();
    ...
    public Double moveFlyingSpeed(final boolean sprinting) {
        double[] speed = moveFlying(0.98f, 0.98f, HackSoar.mc.thePlayer.onGround, 180, sprinting);
    ...
    public void partialStrafeMax(double maxStrafe) {
        double motionX = HackSoar.mc.thePlayer.motionX;
        double motionZ = HackSoar.mc.thePlayer.motionZ;

        MoveUtil.strafe();

        HackSoar.mc.thePlayer.motionX = motionX + Math.max(-maxStrafe, Math.min(maxStrafe, HackSoar.mc.thePlayer.motionX - motionX));
        HackSoar.mc.thePlayer.motionZ = motionZ + Math.max(-maxStrafe, Math.min(maxStrafe, HackSoar.mc.thePlayer.motionZ - motionZ));
    }
    ...
    public void partialStrafePercent(double percentage) {
        ...
        HackSoar.mc.thePlayer.motionX = motionX + (HackSoar.mc.thePlayer.motionX - motionX) * percentage;
        HackSoar.mc.thePlayer.motionZ = motionZ + (HackSoar.mc.thePlayer.motionZ - motionZ) * percentage;
    }

    public double moveMaxFlying(final boolean onGround) {
        float friction = 0.02f;
        final float playerWalkSpeed = HackSoar.mc.thePlayer.getAIMoveSpeed() / 2;
        float strafe = 0.98f;
        float forward = 0.98f;
        float yaw = 180;
    ...
    public float simulationStrafe(float currentMoveYaw) {
        ...
        double lastDeltaX = HackSoar.mc.thePlayer.lastMotionX * friction;
        double lastDeltaZ = HackSoar.mc.thePlayer.lastMotionZ * friction;
        ...
        for (int i = 0; i <= 360; i++) {
            ...
            double deltaX = Math.abs(HackSoar.mc.thePlayer.motionX);
            double deltaZ = Math.abs(HackSoar.mc.thePlayer.motionZ);
    ...
    public void useDiagonalSpeed() {
        KeyBinding[] gameSettings = new KeyBinding[]{HackSoar.mc.gameSettings.keyBindForward, HackSoar.mc.gameSettings.keyBindRight, HackSoar.mc.gameSettings.keyBindBack, HackSoar.mc.gameSettings.keyBindLeft};
        ...
        final double increase = HackSoar.mc.thePlayer.onGround ? groundIncrease : airIncrease;
