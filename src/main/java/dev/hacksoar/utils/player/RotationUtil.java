...
public class RotationUtil {
    ...
    public static Vector2f calculate(final Entity entity) {
        return calculate(entity.getCustomPositionVector().add(0, Math.max(0, Math.min(HackSoar.mc.thePlayer.posY - entity.posY +
                HackSoar.mc.thePlayer.getEyeHeight(), (entity.getEntityBoundingBox().maxY - entity.getEntityBoundingBox().minY) * 0.9)), 0));
    }
    ...
    public Vector2f calculate(final Vec3 to) {
        return calculate(HackSoar.mc.thePlayer.getCustomPositionVector().add(0, HackSoar.mc.thePlayer.getEyeHeight(), 0), new Vector3d(to.xCoord, to.yCoord, to.zCoord));
    }

    public static Vector2f calculate(final Vector3d to) {
        return calculate(HackSoar.mc.thePlayer.getCustomPositionVector().add(0, HackSoar.mc.thePlayer.getEyeHeight(), 0), to);
    }
    ...
    public static Vector2f applySensitivityPatch(final Vector2f rotation) {
        final Vector2f previousRotation = HackSoar.mc.thePlayer.getPreviousRotation();
        final float mouseSensitivity = (float) (HackSoar.mc.gameSettings.mouseSensitivity * (1 + Math.random() / 10000000) * 0.6F + 0.2F);
    ...
    public static Vector2f resetRotation(final Vector2f rotation) {
        ...
        final float yaw = rotation.x + MathHelper.wrapAngleTo180_float(HackSoar.mc.thePlayer.rotationYaw - rotation.x);
        final float pitch = HackSoar.mc.thePlayer.rotationPitch;
        return new Vector2f(yaw, pitch);
    }
