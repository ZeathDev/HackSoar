...
public class RayCastUtil {
    ...
    public static boolean inView(final Entity entity) {
        int renderDistance = 16 * HackSoar.mc.gameSettings.renderDistanceChunks;

        if (entity.threadDistance > 100 || !(entity instanceof EntityPlayer)) {
            Vector2f rotations = RotationUtil.calculate(entity);

            if (Math.abs(MathHelper.wrapAngleTo180_float(rotations.x) - MathHelper.wrapAngleTo180_float(HackSoar.mc.thePlayer.rotationYaw)) > HackSoar.mc.gameSettings.fovSetting) {
                return false;
            }
    ...
    public static MovingObjectPosition rayCast(final Vector2f rotation, final double range, final float expand) {
        return rayCast(rotation, range, expand, HackSoar.mc.thePlayer);
    }

    public static MovingObjectPosition rayCast(final Vector2f rotation, final double range, final float expand, Entity entity) {
        final float partialTicks = HackSoar.mc.timer.renderPartialTicks;
        MovingObjectPosition objectMouseOver;

        if (entity != null && HackSoar.mc.theWorld != null) {
            ...
            final Vec3 vec31 = HackSoar.mc.thePlayer.getVectorForRotation(rotation.y, rotation.x);
            final Vec3 vec32 = vec3.addVector(vec31.xCoord * range, vec31.yCoord * range, vec31.zCoord * range);
            Entity pointedEntity = null;
            Vec3 vec33 = null;
            final float f = 1.0F;
            final List<Entity> list = HackSoar.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * range, vec31.yCoord * range, vec31.zCoord * range).expand(f, f, f), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith));
            double d2 = d1;
    ...
    public static boolean overBlock(final Vector2f rotation, final EnumFacing enumFacing, final BlockPos pos, final boolean strict) {
        final MovingObjectPosition movingObjectPosition = HackSoar.mc.thePlayer.rayTraceCustom(4.5f, rotation.x, rotation.y);
    ...
    public static boolean overBlock(final EnumFacing enumFacing, final BlockPos pos, final boolean strict) {
        final MovingObjectPosition movingObjectPosition = HackSoar.mc.objectMouseOver;
