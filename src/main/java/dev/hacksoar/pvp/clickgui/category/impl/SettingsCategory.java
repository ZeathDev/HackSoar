...
public class SettingsCategory extends Category {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		int offsetX = 14;
		boolean darkMode = HackSoar.instance.settingsManager.getSettingByClass(ClientMod.class, "DarkMode").getValBoolean();
		int colorIndex = 1;
		...
		for(AccentColor c : HackSoar.instance.colorManager.getColors()) {
			...
			boolean selectedColor = HackSoar.instance.colorManager.getClientColor().equals(c);
	...
	public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if(mouseButton == 0) {
			if(MouseUtils.isInside(mouseX, mouseY, this.getX() + 106, this.getY() + 40, 80, 80)) {
				HackSoar.instance.settingsManager.getSettingByClass(ClientMod.class, "DarkMode").setValBoolean(true);
			}
			if(MouseUtils.isInside(mouseX, mouseY, this.getX() + 205, this.getY() + 40, 80, 80)) {
				HackSoar.instance.settingsManager.getSettingByClass(ClientMod.class, "DarkMode").setValBoolean(false);
			}
			for(AccentColor c : HackSoar.instance.colorManager.getColors()) {
				if(MouseUtils.isInside(mouseX, mouseY, this.getX() + 95 + offsetX + scrollAnimation.getValue(), this.getY() + 178 , 20, 20) && canToggle) {
					HackSoar.instance.colorManager.setClientColor(c);
				}
