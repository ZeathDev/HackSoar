...
import dev.hacksoar.utils.SkinUtils;
...
public class GuiAccountManager extends GuiScreen{
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if(!showAddAccount) {
			...
			if(HackSoar.instance.accountManager.getAccounts().isEmpty()) {
        ...
        if(delete) {
        	HackSoar.instance.accountManager.getAccounts().remove(deleteAccount);
			scrollY = 0;
        	delete = false;
        }
		...
		if(mouseButton == 0) {
			...
			if(showAddAccount) {
					...
					new Thread() {
						...
						public void run() {
							...
							try {
								...
								HackSoar.instance.accountManager.getAccounts().add(new Account(AccountType.MICROSOFT, acc.getProfile().getName(), acc.getProfile().getId(), acc.getRefreshToken()));
				...
				if(MouseUtils.isInside(mouseX, mouseY, x + 35, y + 91, 100, 20)) {
					...
					HackSoar.instance.accountManager.getAccounts().add(new Account(AccountType.CRACKED, username, "0", "0"));
				...
				if(MouseUtils.isInside(mouseX, mouseY, (x + width) - 135, y + 91, 100, 20)) {
					if(!usernameField.getText().isEmpty()) {
						HackSoar.instance.accountManager.getAccounts().add(new Account(AccountType.CRACKED, usernameField.getText(), "0", "0"));
				...
				if(!showAddAccount) {
					if(MouseUtils.isInside(mouseX, mouseY, x + 10, y + offsetY + scrollAnimation.getValue(), width - 50, 35)) {
							...
							new Thread() {
								...
								public void run() {
									...
									try {
										...
										(mc).setSession(new Session(acc.getProfile().getName(), acc.getProfile().getId(), acc.getAccessToken(), "legacy"));
										HackSoar.instance.accountManager.setCurrentAccount(HackSoar.instance.accountManager.getAccountByUsername(acc.getProfile().getName()));
						...
						if(a.getAccountType().equals(AccountType.CRACKED)) {
							...
							HackSoar.instance.accountManager.setCurrentAccount(HackSoar.instance.accountManager.getAccountByUsername(a.getUsername()));
						}
					}
				}
	...
	public void keyTyped(char typedChar, int keyCode) {
		...
		if(keyCode == 1) {
			if(showAddAccount) {
				...
				HackSoar.instance.accountManager.save();
